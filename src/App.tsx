...
const formatTabCodeInput = (value: string) => value.toLocaleUpperCase('pt-BR')
...
function App() {
  ...
  const [section, setSection] = useState<SectionId>(() => {
    const party = initialAppData.parties.find((item) => item.id === initialAppData.selectedPartyId && item.active && !item.archived) ?? initialAppData.parties.find((item) => item.active && !item.archived)
    const hasActiveTab = party?.tabs.some((tab) => tab.active) ?? false
    const hasActiveMenuItem = party?.menu.some((item) => item.active) ?? false

    return party && hasActiveTab && hasActiveMenuItem ? 'consumption' : 'parties'
  })
  ...
  const selectedTab = activeTabs.some((tab) => tab.id === appData.selectedTabId) ? appData.selectedTabId ?? '' : activeTabs[0]?.id ?? ''
  ...
  const selectedTabConsumed = selectedTabConsumptions.reduce((sum, item) => sum + item.price, 0)
  const selectedTabRemaining = selectedActiveTab ? Math.max(selectedActiveTab.minimumSpend - selectedTabConsumed, 0) : 0
  ...
  const hasActiveTabsAndMenu = activeTabs.length > 0 && activeMenu.length > 0
  ...
  const activePartyHeader = activeParty ? [
    ...
    section === 'consumption' && selectedActiveTab ? selectedTabRemaining > 0 ? `A consumir ${formatCurrency(selectedTabRemaining)}` : 'Saldo mínimo já consumido' : null,
  ...
  function archiveParty(partyId: string) {
    updateParty(partyId, (p) => ({ ...p, archived: true, active: false }))
    setModal(null)
  }

  function unarchiveParty(partyId: string) {
    setAppData((current) => normalizeAppData({
      ...current,
      selectedPartyId: partyId,
      parties: current.parties.map((party) => ({ ...party, archived: party.id === partyId ? false : party.archived, active: party.id === partyId })),
    }))
    setModal(null)
  }
  ...
  function deleteParty(partyId: string) {
    ...
    if (!party?.archived || !confirmDeletion('Tem certeza que deseja excluir esta festa? Esta ação vai excluir todas comandas, itens e consumo registrado.')) return
    setAppData((current) => normalizeAppData({ ...current, parties: current.parties.filter((p) => p.id !== partyId) }))
    setModal(null)
  }

  function deleteTab(tabId: string) {
    if (!activeParty || !confirmDeletion('Tem certeza que deseja excluir esta comanda? Os consumos relacionados também serão removidos.')) return
    updateParty(activeParty.id, (p) => ({ ...p, tabs: p.tabs.filter((t) => t.id !== tabId), consumptions: p.consumptions.filter((c) => c.tabId !== tabId) }))
    setModal(null)
  }

  function deleteMenuItem(itemId: string) {
    if (!activeParty || !confirmDeletion('Tem certeza que deseja excluir este item? Os consumos relacionados também serão removidos.')) return
    updateParty(activeParty.id, (p) => ({ ...p, menu: p.menu.filter((m) => m.id !== itemId), consumptions: p.consumptions.filter((c) => c.menuItemId !== itemId) }))
    setModal(null)
  }

  function deleteConsumption(consumptionId: string) {
    if (!activeParty || !confirmDeletion('Tem certeza que deseja remover este consumo?')) return
    updateParty(activeParty.id, (p) => ({ ...p, consumptions: p.consumptions.filter((c) => c.id !== consumptionId) }))
  }

  function toggleTabActive(tab: Tab) {
    const willDeactivate = tab.active
    const hasConsumptions = activeParty?.consumptions.some((consumption) => consumption.tabId === tab.id) ?? false

    if (willDeactivate && hasConsumptions && !window.confirm('Esta comanda já possui consumação. Deseja desativá-la mesmo assim?')) return

    if (!activeParty) return
    updateParty(activeParty.id, (p) => ({ ...p, tabs: p.tabs.map((t) => t.id === tab.id ? { ...t, active: !t.active } : t) }))
    setModal(null)
  }
  ...
  function saveTab(event: FormEvent) {
    ...
    const payload = { code: tabForm.code.trim().slice(0, LIMITS.tabCode), nfcCard: formatTabCodeInput(tabForm.nfcCard).trim().slice(0, LIMITS.tabNfcCard), minimumSpend }
  ...
  function saveMenu(event: FormEvent) {
    event.preventDefault()
    const price = menuForm.price.trim() ? parseCurrencyInput(menuForm.price) : 0
  ...
  return (
    <main className="app-shell" onCopy={(event) => event.preventDefault()} onCut={(event) => event.preventDefault()}>
        ...
        <div className="layout">
          ...
          <div className="content">
            {section === 'parties' && <Section title="Festas" actions={<div className="party-actions"><Button onClick={() => openPartyForm()}>Criar festa</Button></div>}>
              {visibleParties.length === 0 ? <div className="empty-state"><p>não existe nenhuma festa cadastrada e desarquivada</p></div> : <div className="grid-list">{visibleParties.map((party) => <button className="card card-button" key={party.id} onClick={() => openPartyDetails(party.id)}><div className="party-card-content"><div className="party-card-main"><h3>{party.name}</h3><p>{formatStoredDate(party.date)}</p></div><div className="badges"><span className={party.active ? 'badge-active' : 'badge-inactive'}>{party.active ? 'Ativa' : 'Inativa'}</span></div></div></button>)}</div>}
              <div className="toggle-panel archived-toggle-panel"><Button variant="secondary" onClick={() => setShowArchived((value) => !value)}>{showArchived ? 'Ocultar festas arquivada' : 'Mostrar festas arquivada'}</Button>{showArchived && (archivedParties.length === 0 ? <div className="empty-state compact-state"><p>não existe nenhuma festa arquivada</p></div> : <div className="grid-list">{archivedParties.map((party) => <button className="card card-button" key={party.id} onClick={() => openPartyDetails(party.id)}><div className="party-card-content"><div className="party-card-main"><h3>{party.name}</h3><p>{formatStoredDate(party.date)}</p></div><div className="badges"><span className="badge-archived">Arquivada</span></div></div></button>)}</div>)}</div>
            </Section>}

            {section === 'tabs' && activeParty && <Section title="Comandas" actions={<div className="party-actions"><Button onClick={() => openTabForm()}>Criar comanda</Button></div>}>{activeParty.tabs.length === 0 ? <div className="empty-state"><p>não existe nenhuma comanda criada</p></div> : <div className="grid-list">{activeParty.tabs.map((tab) => <button className="card card-button" key={tab.id} onClick={() => openTabDetails(tab.id)}><div className="item-card-content"><div><h3>{tab.code}</h3><p>{tab.nfcCard && <><strong className="tab-card-code">{tab.nfcCard}</strong> • </>}Mínimo {formatCurrency(tab.minimumSpend)}</p></div><div className="badges"><span className={tab.active ? 'badge-active' : 'badge-inactive'}>{tab.active ? 'Ativa' : 'Inativa'}</span></div></div></button>)}</div>}</Section>}
            ...
            {section === 'balances' && activeParty && <Section title="Saldos"><div className="totals"><strong>Total consumido: {formatCurrency(totals.consumed)}</strong><strong>Total a consumir: {formatCurrency(totals.minimum)}</strong></div>{balanceTabs.length === 0 ? <div className="empty-state"><p>não existe nenhuma comanda ativa</p></div> : <div className="grid-list">{balanceTabs.map((tab) => { const consumed = activeParty.consumptions.filter((item) => item.tabId === tab.id).reduce((sum, item) => sum + item.price, 0); const remaining = Math.max(tab.minimumSpend - consumed, 0); return <article className="card" key={tab.id}><div><h3>{tab.code}</h3><p className="balance-summary"><span>Consumido {formatCurrency(consumed)}</span><span>{remaining > 0 ? `A consumir ${formatCurrency(remaining)}` : 'Mínimo consumido'}</span></p></div></article> })}</div>}</Section>}
          </div>
        </div>
      </div>
      ...
      <Modal title={editingId ? 'Editar comanda' : 'Nova comanda'} open={modal === 'tab'} onClose={() => setModal(null)}><form onSubmit={saveTab} className="form"><input maxLength={20} placeholder="Responsável da comanda" value={tabForm.code} onChange={(e) => setTabForm({ ...tabForm, code: e.target.value })} required /><input maxLength={10} placeholder="Código da comanda" value={tabForm.nfcCard} onChange={(e) => setTabForm({ ...tabForm, nfcCard: formatTabCodeInput(e.target.value) })} /><input maxLength={12} min="0" step="0.01" type="text" placeholder="Consumo mínimo" value={tabForm.minimumSpend} onChange={(e) => setTabForm({ ...tabForm, minimumSpend: e.target.value })} inputMode="decimal" pattern="[0-9.]+(,[0-9]{1,2})?" /><Button type="submit">Salvar</Button></form></Modal>
      <Modal title={editingId ? 'Editar item' : 'Novo item'} open={modal === 'menu'} onClose={() => setModal(null)}><form onSubmit={saveMenu} className="form"><input maxLength={20} placeholder="Nome do item" value={menuForm.name} onChange={(e) => setMenuForm({ ...menuForm, name: e.target.value })} required /><input maxLength={12} min="0" step="0.01" type="text" placeholder="Preço (opcional)" value={menuForm.price} onChange={(e) => setMenuForm({ ...menuForm, price: e.target.value })} inputMode="decimal" pattern="[0-9.]+(,[0-9]{1,2})?" /><Button type="submit">Salvar</Button></form></Modal>
    </main>
  )
}

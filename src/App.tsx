      ...
      <div className="app-container">
        ...
        <div className="layout">
          ...
          <div className="content">
            {section === 'parties' && <Section title="Festas" subtitle="Gerencie eventos" actions={<div className="party-actions"><Button onClick={() => openPartyForm()}>Criar festa</Button><Button variant="secondary" onClick={() => setShowArchived((value) => !value)}>{showArchived ? 'Ocultar festas arquivadas' : 'Mostrar festas arquivadas'}</Button></div>}>
              {visibleParties.length === 0 ? <div className="empty-state"><strong>Nenhuma festa criada ainda</strong><p>Crie a primeira festa para liberar comandas, cardápios, consumo e saldos.</p></div> : <div className="grid-list">{visibleParties.map((party) => <button className="card card-button" key={party.id} onClick={() => openPartyDetails(party.id)}><div className="party-card-content"><h3>{party.name}</h3><div className="party-card-meta"><p>{formatStoredDate(party.date)}</p><div className="badges"><span className={party.active ? 'badge-active' : 'badge-inactive'}>{party.active ? 'Ativa' : 'Inativa'}</span>{party.archived && <span className="badge-archived">Arquivada</span>}</div></div></div></button>)}</div>}
            </Section>}

            {section === 'tabs' && activeParty && <Section title="Comandas" subtitle={activeParty.name} actions={<Button onClick={() => openTabForm()}>Criar comanda</Button>}>{activeParty.tabs.length === 0 ? <div className="empty-state"><strong>Nenhuma comanda criada para esta festa</strong><p>Adicione comandas para registrar consumo e acompanhar saldos.</p></div> : <div className="grid-list">{activeParty.tabs.map((tab) => <button className="card card-button" key={tab.id} onClick={() => openTabDetails(tab.id)}><div><h3>{tab.code}</h3><p>Mínimo {formatCurrency(tab.minimumSpend)}</p><div className="badges"><span className={tab.active ? 'badge-active' : 'badge-inactive'}>{tab.active ? 'Ativa' : 'Inativa'}</span></div></div></button>)}</div>}</Section>}

            {section === 'menu' && activeParty && <Section title="Cardápios" subtitle={activeParty.name} actions={<Button onClick={() => openMenuForm()}>Criar item</Button>}>{activeParty.menu.length === 0 ? <div className="empty-state"><strong>Nenhum item criado para esta festa</strong><p>Monte o cardápio para começar a registrar consumo.</p></div> : <div className="grid-list">{activeParty.menu.map((item) => <button className="card card-button" key={item.id} onClick={() => openMenuDetails(item.id)}><div><h3>{item.name}</h3><p>{formatCurrency(item.price)}</p><div className="badges"><span className={item.active ? 'badge-active' : 'badge-inactive'}>{item.active ? 'Ativo' : 'Inativo'}</span></div></div></button>)}</div>}</Section>}

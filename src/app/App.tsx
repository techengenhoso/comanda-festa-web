...
import { formatTabCodeInput, newMenu, newParty, newTab } from "../features/forms/ui/forms"
import { DEFAULT_MENU_ITEMS } from "../features/party/model/constants"
import { LIMITS, normalizeAppData } from "../features/party/model/normalization"
import {
  findActiveParty,
  findPartyItem,
  getActiveMenu,
...
import { BalancesPage, ConsumptionPage, MenuPage, PartiesPage, TabsPage } from "./pages"
...
function App() {
  ...
  const { visibleParties, archivedParties } = splitPartiesByArchive(appData.parties)
  ...
  const selectedTabRemaining = getRemainingMinimum(activeParty, selectedActiveTab)
  const selectedPartyDetail = findPartyItem(appData.parties, selectedPartyDetailId)
  const selectedTabDetail = findPartyItem(activeParty?.tabs, selectedTabDetailId)
  const selectedMenuDetail = findPartyItem(activeParty?.menu, selectedMenuDetailId)
  ...
  function openMenuForm(item?: MenuItem) {
    ...
    setMenuForm(
      item ? { name: item.name, price: formatCurrencyInput(item.price) } : newMenu()
    )
    setModal("menu")
  }
  ...
  function saveTab(event: FormEvent) {
    ...
    const payload = {
      ...
      nfcCard: formatTabCodeInput(tabForm.nfcCard).trim().slice(0, LIMITS.tabNfcCard),
      minimumSpend,
    }
    updateParty(activeParty.id, party => ({
      ...party,
      tabs: editingId
        ? party.tabs.map(tab => (tab.id === editingId ? { ...tab, ...payload } : tab))
        : [
            ...party.tabs,
            {
              id: createId(),
  ...
  function saveMenu(event: FormEvent) {
    ...
    updateParty(activeParty.id, party => ({
      ...party,
      menu: editingId
        ? party.menu.map(item => (item.id === editingId ? { ...item, ...payload } : item))
        : [
            ...party.menu,
            {
              id: createId(),
  ...
  function deleteTab(tabId: string) {
    ...
    updateParty(activeParty.id, party => ({
      ...
      consumptions: party.consumptions.filter(consumption => consumption.tabId !== tabId),
    }))
    setModal(null)
  }
  ...
  function toggleTabActive(tab: Tab) {
    const hasConsumptions =
      activeParty?.consumptions.some(consumption => consumption.tabId === tab.id) ?? false
    if (
      tab.active &&
      hasConsumptions &&
      !window.confirm("Esta comanda já possui consumação. Deseja desativá-la mesmo assim?")
    )
      return
  ...
  function toggleMenuItemActive(item: MenuItem) {
    ...
    updateParty(activeParty.id, party => ({
      ...
      menu: party.menu.map(menuItem =>
        menuItem.id === item.id ? { ...menuItem, active: !menuItem.active } : menuItem
      ),
    }))
    setModal(null)
  }
  ...
  function deleteConsumption(consumptionId: string) {
    if (!activeParty || !window.confirm("Tem certeza que deseja remover este consumo?"))
      return
    updateParty(activeParty.id, party => ({
      ...party,
      consumptions: party.consumptions.filter(
    ...
    >
      <div className="app-container">
        ...
        <div className="layout">
          <Nav current={section} disabled={!activeParty} onChange={setSection} />
...
function getInitialSection(appData: AppData): SectionId {
  ...
  return party && hasActiveTabsAndMenu(getActiveTabs(party), getActiveMenu(party))
    ? "consumption"
    : "parties"
}
